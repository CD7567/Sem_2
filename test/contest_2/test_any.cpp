...
class Dummy {
 public:
  Dummy(int, int) {
    message = "int, int dummy";
  }
  Dummy(char, char) {
    message = "char, char dummy";
  }
  Dummy(char, int) {
    message = "char, int dummy";
  }
  Dummy(int, char) {
    message = "int, char dummy";
  }
...
TEST_CASE("MakeAny") {
  auto dummy_1 = MakeAny<Dummy>(1, 2);
  auto dummy_2 = MakeAny<Dummy>(1, 'a');
  auto dummy_3 = MakeAny<Dummy>('a', 2);
  auto dummy_4 = MakeAny<Dummy>('a', 'b');

  std::cout << AnyCast<Dummy>(dummy_1).message << std::endl
            << AnyCast<Dummy>(dummy_2).message << std::endl
            << AnyCast<Dummy>(dummy_3).message << std::endl
            << AnyCast<Dummy>(dummy_4).message << std::endl;
}