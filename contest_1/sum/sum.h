...
class LightNumBuffer {
 public:
  ...
  LightNumBuffer operator+(const LightNumBuffer& rhs) {
    LightNumBuffer res(std::max(size_, rhs.size_) + 1);
    size_t sum_bound = std::min(size_, rhs.size_);
    ...
    for (size_t i = sum_bound; i < size_; ++i) {
      res.array_[i] = array_[i];
    }

    for (size_t i = sum_bound; i < rhs.size_; ++i) {
