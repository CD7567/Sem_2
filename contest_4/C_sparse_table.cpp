...
int main() {
  ...
  std::cin >> characters_amount >> comm_amount;

  const size_t table_height = Log(characters_amount);

  std::vector<int> characters;

  characters.reserve(characters_amount + 2);
  ...
  for (size_t i = 0; i < comm_amount; ++i) {
    ...
    std::cout << std::min(sparse_table[height][left], sparse_table[height][right - (1 << height) + 1]) << std::endl;
  }
}